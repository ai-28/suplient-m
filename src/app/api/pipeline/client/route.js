...
export async function GET(request) {
    try {
        ...
        // Get client pipeline stages for the coach
        const stages = await sql`
      SELECT * FROM "ClientPipelineStage" 
    ...
    `;
        ...
        console.error('Error fetching client pipeline stages:', error);
        return NextResponse.json(
...
export async function POST(request) {
    try {
        ...
        await sql.begin(async (sql) => {
            ...
            await sql`
        DELETE FROM "ClientPipelineStage" 
        WHERE "coachId" = ${session.user.id}
      `;
        ...
        return NextResponse.json({
            success: true,
            message: 'Client pipeline stages saved successfully'
        });
    } catch (error) {
        console.error('Error saving client pipeline stages:', error);
        return NextResponse.json(
